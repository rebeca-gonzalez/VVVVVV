...
PlatformBlueVer::PlatformBlueVer(int speed)
{
	speedY = speed;
}


PlatformBlueVer::~PlatformBlueVer()
{
}

void PlatformBlueVer::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram) {
...
int PlatformBlueVer::abs(int a) {
	if (a > 0) return a;
	return a * -1;
}

int PlatformBlueVer::min(int a, int b) {
	if (a < b) return a;
	else return b;
}

int PlatformBlueVer::update(int deltaTime, glm::vec2 upPos, glm::vec2 lowPos, int dir) {
...
bool PlatformBlueVer::checkCollision(glm::vec2 upPosPlayer, glm::vec2 lowPosPlayer, glm::vec2 upPosCollider, glm::vec2 lowPosCollider, int dir) {
	if (
		...
		)
	{
		...
		if (dif_clipping > 9) return false;
		return true;
	}
	return false;
}