...
PlatformGreenHor::PlatformGreenHor(int speed)
{
	speedX = speed;
}


PlatformGreenHor::~PlatformGreenHor()
{
}

void PlatformGreenHor::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram) {
	character_size = glm::ivec2(64, 16);
	collision_value = 2;
	spritesheet.loadFromFile("images/green_platform.png", TEXTURE_PIXEL_FORMAT_RGBA);
	sprite = Sprite::createSprite(character_size, glm::vec2(1.f, 1.f), &spritesheet, &shaderProgram);
	Enemy::init(tileMapPos, shaderProgram);
}

int PlatformGreenHor::abs(int a) {
	if (a > 0) return a;
	return a * -1;
}

int PlatformGreenHor::min(int a, int b) {
	if (a < b) return a;
	else return b;
}

int PlatformGreenHor::update(int deltaTime, glm::vec2 upPos, glm::vec2 lowPos, int dir) {
...
bool PlatformGreenHor::checkCollision(glm::vec2 upPosPlayer, glm::vec2 lowPosPlayer, glm::vec2 upPosCollider, glm::vec2 lowPosCollider, int dir) {
	if (
